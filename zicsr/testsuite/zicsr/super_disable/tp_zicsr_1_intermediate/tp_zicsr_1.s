...
SID_ZICSR_12:
	...
	csrrsi s1, scause, 0x1f
	li sp, tp_csr_storage
	ld t2, 0(sp)
	;#csr_rw(scause, write, false, true)
SID_ZICSR_12_passed:
	;#test_passed()
...
SID_ZICSR_16:
	...
	li a5, 0x3b1a11e2
	addi a6, a5, 0x43b
	csrrw s10, stvec, zero
	li s7, 0xad45f240
	li s9, 0x380208ac
	mulhsu s4, s7, s9
	csrrs t5, senvcfg, zero
	li t2, 0xc2cd789d
	li t3, 0x75a89297
	li a4, 0xf3c64afa
	fcvt.d.lu fs8, t2
...
SID_ZICSR_17:
	...
	csrrs a4, stvec, zero
	csrrw t4, stvec, zero
	csrrc a2, scause, zero
	csrrw s11, stval, zero
	csrrwi t5, senvcfg, 0
	csrrw a3, scause, zero
	csrrsi s1, sscratch, 0
	csrrw s1, scause, zero
	csrrci s9, senvcfg, 0
	csrrs s9, sepc, zero
	csrrw s3, scounteren, zero
	csrrs s0, sscratch, zero
	csrrs a3, scounteren, zero
	csrrs a1, sepc, zero
	csrrc a5, stvec, zero
	csrrs t2, stval, zero
	csrrwi t4, scounteren, 0
	csrrs a0, scounteren, zero
	csrrsi t4, senvcfg, 0
	csrrs a0, scounteren, zero
	csrrci s10, sepc, 0
	csrrc s8, senvcfg, zero
	csrrw t6, scause, zero
	csrrc s4, stval, zero
	csrrs s4, senvcfg, zero
	csrrc t2, sscratch, zero
	csrrc s7, stval, zero
	csrrc s1, stvec, zero
	csrrwi s11, sscratch, 0
	csrrc s2, sepc, zero
	csrrsi t1, scounteren, 0
	csrrc a4, sscratch, zero
	csrrci s3, scause, 0
	csrrwi t6, stvec, 0
	csrrw a2, sepc, zero
	csrrwi t1, senvcfg, 0
	csrrs a3, senvcfg, zero
	csrrwi a4, senvcfg, 0
	csrrc a2, scause, zero
	csrrwi a5, scause, 0
	csrrwi t2, senvcfg, 0
	csrrwi a4, scounteren, 0
	csrrsi a0, scounteren, 0
	csrrwi s11, senvcfg, 0
	csrrci s2, scounteren, 0
	csrrsi s6, sepc, 0
	csrrw a6, sepc, zero
	csrrsi s8, stvec, 0
	csrrs s3, stvec, zero
	...
	csrrwi a1, scause, 0
	csrrsi s4, sepc, 0
	csrrsi t5, scounteren, 0
	csrrsi t6, senvcfg, 0
	csrrci s4, stvec, 0
	csrrci s6, senvcfg, 0
	csrrw s10, sscratch, zero
	csrrci a3, senvcfg, 0
	csrrs s11, stval, zero
	csrrci a7, stvec, 0
	csrrc a2, scounteren, zero
	csrrci s5, scounteren, 0
	csrrwi s8, scause, 0
	csrrci s1, stval, 0
	csrrsi s9, sscratch, 0
	csrrci s8, stval, 0
	csrrci t2, stvec, 0
SID_ZICSR_17_passed:
	;#test_passed()
...
SID_ZICSR_19:
	...
	csrrw t5, sscratch, s4
	csrr s3, sscratch
	beq s3, s4, pass_label_0
	li s10, failed_addr
	ld a3, 0(s10)
	jr a3
pass_label_0:
	...
	li s3, 0
	csrrw s3, sscratch, s3
	csrr s11, sscratch
	beq s11, t4, pass_label_1
	li s8, failed_addr
	ld a4, 0(s8)
	jr a4
pass_label_1:
	# CSRRS: Set all bits in sscratch
	csrrs t5, sscratch, s4
	csrr s8, sscratch
	beq s8, s4, pass_label_2
	li t1, failed_addr
	ld s9, 0(t1)
	jr s9
pass_label_2:
	# CSRRC: Clear all bits in sscratch
	csrrc t1, sscratch, s4
	csrr a4, sscratch
	beq a4, t4, pass_label_3
	li s5, failed_addr
	ld t1, 0(s5)
	jr t1
pass_label_3:
	# CSRRWI: Write max immediate (0x1F) to sscratch
	csrrwi s4, sscratch, 0x1f
	csrr s10, sscratch
	beq s10, s7, pass_label_4
	li s0, failed_addr
	ld s0, 0(s0)
	jr s0
pass_label_4:
	# CSRRWI: Clear sscratch by writing 0
	csrrwi s6, sscratch, 0
	csrr a2, sscratch
	beq a2, t4, pass_label_5
	li a7, failed_addr
	ld s1, 0(a7)
	jr s1
pass_label_5:
	# CSRRSI: Set bits with max immediate (0x1F) in sscratch
	csrrsi s3, sscratch, 0x1f
	csrr a1, sscratch
	beq a1, s7, pass_label_6
	li a5, failed_addr
	ld s8, 0(a5)
	jr s8
pass_label_6:
	# CSRRCI: Clear bits with max immediate (0x1F) in sscratch
	csrrci s9, sscratch, 0x1f
	csrr s8, sscratch
	beq s8, t4, pass_label_7
	li t6, failed_addr
	ld s0, 0(t6)
	jr s0
pass_label_7:
...
pass_label_12:
	...
	csrr t2, fcsr
	and s1, t2, t1
	beq s1, a4, pass_label_13
	li a5, failed_addr
	ld s8, 0(a5)
	jr s8
pass_label_13:
	# CSRRSI: Set bits with max immediate (0x1F) in fcsr
	csrrsi s11, fcsr, 0x1f
	csrr a1, fcsr
	and t6, a1, t1
	beq t6, a0, pass_label_14
	li s10, failed_addr
	ld a7, 0(s10)
	jr a7
pass_label_14:
	# CSRRCI: Clear bits with max immediate (0x1F) in fcsr
	csrrci s11, fcsr, 0x1f
	csrr s9, fcsr
	and s0, s9, t1
	beq s0, a4, pass_label_15
	li s1, failed_addr
	ld s5, 0(s1)
	jr s5
pass_label_15:
