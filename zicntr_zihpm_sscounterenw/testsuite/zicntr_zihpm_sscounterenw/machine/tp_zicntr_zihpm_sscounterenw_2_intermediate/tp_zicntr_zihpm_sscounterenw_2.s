...
SID_XCOUNTEREN_01_M:
	...
	andi sp, sp, -16
	li t2, 0xffffffff
	csrrs s1, mcounteren, t2
	li a0, 0xffffffff
	csrrs s4, scounteren, a0
...
SID_XCOUNTEREN_02_M:
	...
	andi sp, sp, -16
	li a1, 0xffffffff
	csrrs s8, mcounteren, a1
	li a2, 0xffffffff
	csrrc a5, scounteren, a2
	# Test each CSR in M mode - all should succeed
	csrr a6, cycle
	csrr t1, time
	csrr a4, instret
...
SID_XCOUNTEREN_03_M:
	...
	andi sp, sp, -16
	li s6, 0xffffffff
	csrrc a3, mcounteren, s6
	li s7, 0xffffffff
	csrrs a5, scounteren, s7
	# Test each CSR in M mode - all should succeed
	csrr s9, cycle
	csrr s11, time
	csrr t3, instret
...
SID_XCOUNTEREN_04_M:
	...
	andi sp, sp, -16
	li s10, 0xffffffff
	csrrc s9, mcounteren, s10
	li s8, 0xffffffff
	csrrc s3, scounteren, s8
	# Test each CSR in M mode - all should succeed
	csrr a5, cycle
	csrr s8, time
	csrr a2, instret
...
SID_XCOUNTEREN_06_M:
	...
	csrrwi a0, mcounteren, 0x1
	csrrwi t1, scounteren, 0x1
	# Test the enabled CSR in M mode - should succeed
	csrr s0, cycle
	# Test other CSRs in M mode - should also succeed
	csrr s3, time
	...
	# Set only this field in both registers, clear all others
	csrrwi t3, mcounteren, 0x4
	csrrwi s1, scounteren, 0x4
	# Test the enabled CSR in M mode - should succeed
	csrr a1, instret
	# Test other CSRs in M mode - should also succeed
	csrr s6, cycle
	...
	# Set only this field in both registers, clear all others
	csrrwi a6, mcounteren, 0x10
	csrrwi s1, scounteren, 0x10
	# Test the enabled CSR in M mode - should succeed
	csrr t4, hpmcounter4
	# Test other CSRs in M mode - should also succeed
	csrr t6, cycle
	...
	# Set only this field in both registers, clear all others
	li s6, 0x20
	csrrw a3, mcounteren, s6
	li s9, 0x20
	csrrw a1, scounteren, s9
	# Test the enabled CSR in M mode - should succeed
	csrr a5, hpmcounter5
	# Test other CSRs in M mode - should also succeed
	csrr s5, cycle
	...
	# Set only this field in both registers, clear all others
	li s2, 0x80
	csrrw s1, mcounteren, s2
	li t5, 0x80
	csrrw a7, scounteren, t5
	# Test the enabled CSR in M mode - should succeed
	csrr s4, hpmcounter7
	# Test other CSRs in M mode - should also succeed
	csrr a1, cycle
	...
	# Set only this field in both registers, clear all others
	li t6, 0x100
	csrrw a7, mcounteren, t6
	li s0, 0x100
	csrrw a3, scounteren, s0
	# Test the enabled CSR in M mode - should succeed
	csrr a0, hpmcounter8
	# Test other CSRs in M mode - should also succeed
	csrr s10, cycle
	...
	# Set only this field in both registers, clear all others
	li s6, 0x200
	csrrw a0, mcounteren, s6
	li s8, 0x200
	csrrw s3, scounteren, s8
	# Test the enabled CSR in M mode - should succeed
	csrr s8, hpmcounter9
	# Test other CSRs in M mode - should also succeed
	csrr a4, cycle
	...
	# Set only this field in both registers, clear all others
	li t2, 0x400
	csrrw t2, mcounteren, t2
	li t5, 0x400
	csrrw s10, scounteren, t5
	# Test the enabled CSR in M mode - should succeed
	csrr s10, hpmcounter10
	# Test other CSRs in M mode - should also succeed
	csrr a7, cycle
	...
	# Set only this field in both registers, clear all others
	li t4, 0x800
	csrrw s0, mcounteren, t4
	li a3, 0x800
	csrrw s10, scounteren, a3
	# Test the enabled CSR in M mode - should succeed
	csrr t1, hpmcounter11
	# Test other CSRs in M mode - should also succeed
	csrr s2, cycle
	...
	# Set only this field in both registers, clear all others
	li s6, 0x1000
	csrrw t5, mcounteren, s6
	li t4, 0x1000
	csrrw a1, scounteren, t4
	# Test the enabled CSR in M mode - should succeed
	csrr s10, hpmcounter12
	# Test other CSRs in M mode - should also succeed
	csrr s5, cycle
	...
	# Set only this field in both registers, clear all others
	li a3, 0x2000
	csrrw s6, mcounteren, a3
	li s8, 0x2000
	csrrw s5, scounteren, s8
	# Test the enabled CSR in M mode - should succeed
	csrr s3, hpmcounter13
	# Test other CSRs in M mode - should also succeed
	csrr a7, cycle
	...
	# Set only this field in both registers, clear all others
	li s3, 0x8000
	csrrw s4, mcounteren, s3
	li s9, 0x8000
	csrrw s10, scounteren, s9
	# Test the enabled CSR in M mode - should succeed
	csrr s11, hpmcounter15
	# Test other CSRs in M mode - should also succeed
	csrr s7, cycle
	...
	# Set only this field in both registers, clear all others
	li s2, 0x10000
	csrrw s3, mcounteren, s2
	li s4, 0x10000
	csrrw a1, scounteren, s4
	# Test the enabled CSR in M mode - should succeed
	...
	# Set only this field in both registers, clear all others
	li a0, 0x20000
	csrrw t2, mcounteren, a0
	li s2, 0x20000
	csrrw a3, scounteren, s2
	# Test the enabled CSR in M mode - should succeed
	csrr a7, hpmcounter17
	# Test other CSRs in M mode - should also succeed
	csrr t5, cycle
	...
	# Set only this field in both registers, clear all others
	li t6, 0x40000
	csrrw s6, mcounteren, t6
	li t3, 0x40000
	csrrw s5, scounteren, t3
	# Test the enabled CSR in M mode - should succeed
	csrr a3, hpmcounter18
	# Test other CSRs in M mode - should also succeed
	csrr t1, cycle
	...
	# Set only this field in both registers, clear all others
	li a1, 0x80000
	csrrw t2, mcounteren, a1
	li a5, 0x80000
	csrrw s8, scounteren, a5
	# Test the enabled CSR in M mode - should succeed
	csrr a7, hpmcounter19
	# Test other CSRs in M mode - should also succeed
	csrr s0, cycle
	...
	# Set only this field in both registers, clear all others
	li s0, 0x400000
	csrrw a3, mcounteren, s0
	li t3, 0x400000
	csrrw s6, scounteren, t3
	# Test the enabled CSR in M mode - should succeed
	csrr t4, hpmcounter22
	# Test other CSRs in M mode - should also succeed
	csrr t6, cycle
	...
	# Set only this field in both registers, clear all others
	li s7, 0x800000
	csrrw s4, mcounteren, s7
	li a1, 0x800000
	csrrw a6, scounteren, a1
	# Test the enabled CSR in M mode - should succeed
	csrr t2, hpmcounter23
	# Test other CSRs in M mode - should also succeed
	csrr t6, cycle
	...
	# Set only this field in both registers, clear all others
	li s2, 0x2000000
	csrrw a6, mcounteren, s2
	li s3, 0x2000000
	csrrw a6, scounteren, s3
	# Test the enabled CSR in M mode - should succeed
	csrr s7, hpmcounter25
	# Test other CSRs in M mode - should also succeed
	csrr a3, cycle
	...
	# Set only this field in both registers, clear all others
	li t2, 0x8000000
	csrrw s0, mcounteren, t2
	li a5, 0x8000000
	csrrw s5, scounteren, a5
	# Test the enabled CSR in M mode - should succeed
	csrr s1, hpmcounter27
	# Test other CSRs in M mode - should also succeed
	csrr t3, cycle
	...
	# Set only this field in both registers, clear all others
	li a2, 0x10000000
	csrrw s2, mcounteren, a2
	li t4, 0x10000000
	csrrw t4, scounteren, t4
	# Test the enabled CSR in M mode - should succeed
	csrr a7, hpmcounter28
	# Test other CSRs in M mode - should also succeed
	csrr a7, cycle
	...
	# Set only this field in both registers, clear all others
	li s10, 0x20000000
	csrrw t6, mcounteren, s10
	li t5, 0x20000000
	csrrw s4, scounteren, t5
	# Test the enabled CSR in M mode - should succeed
	csrr s1, hpmcounter29
	# Test other CSRs in M mode - should also succeed
	csrr a7, cycle
	...
	# Set only this field in both registers, clear all others
	li s0, 0x40000000
	csrrw a4, mcounteren, s0
	li a7, 0x40000000
	csrrw t3, scounteren, a7
	# Test the enabled CSR in M mode - should succeed
	csrr a7, hpmcounter30
	# Test other CSRs in M mode - should also succeed
	csrr s3, cycle
	...
	# Set only this field in both registers, clear all others
	li a4, 0x80000000
	csrrw a5, mcounteren, a4
	li a1, 0x80000000
	csrrw t5, scounteren, a1
	# Test the enabled CSR in M mode - should succeed
	csrr t3, hpmcounter31
	# Test other CSRs in M mode - should also succeed
	csrr t3, cycle
